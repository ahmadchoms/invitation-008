...
export function RsvpSection() {
  const handleRsvp = () => {
    const encodedMessage = encodeURIComponent(weddingData.contact.defaultMessage);
    window.open(`https://wa.me/${weddingData.contact.whatsapp}?text=${encodedMessage}`, '_blank');
  };
  ...
  return (
    <section className="py-24 px-6 md:py-32 relative bg-muted/20 text-center overflow-hidden">
      ...
      >
        <h2 className="font-heading text-3xl md:text-4xl text-primary mb-4">Konfirmasi Kehadiran</h2>
        <p className="text-secondary-foreground mb-10 text-sm md:text-base leading-relaxed">
        ...
        >
          ...
          <span className="relative">Konfirmasi via WhatsApp</span>
        </button>
      </motion.div>
    </section>
  );
